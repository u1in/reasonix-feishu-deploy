...
| `--acknowledge-risk` | 确认已了解「允许所有用户」的安全风险（静默模式下必填） |
| `--yes` / `-y` | 跳过所有确认提示 |
...
# 静默卸载 + 全量清理
npx @u1in/reasonix-feishu-deploy --undeploy --yes \
  --remove-sessions --force-remove-dir
```
...
| `--restore-user-config` / `--no-restore-user-config` | 还原 `~/.reasonix/config.toml`（默认是，有备份时） |
| `--yes` / `-y` | 跳过所有确认提示 |

> 卸载时如果检测到 `~/.reasonix/config.toml.deploy-bak`（安装前的备份），会询问是否还原。选择「还原」则恢复安装前的个人配置；选择「保留」则保留合并后的配置和备份文件。
...
配置加载优先级：flag > `./reasonix.toml`（项目级） > `~/.reasonix/config.toml`（用户级） > 内置默认值。
